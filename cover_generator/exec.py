...
def init_qtgui(display=None, style=None, qtargs=None):
	...
	app_icon.addFile(dir+'/ressources/icon32x32.png', QSize(32,32))
...
def main():
	...
	parser.add_option(
		"-x", "--xvfb", nargs=2, type="int", dest="xvfb",
	...
	parser.add_option(
		"-f", "--format", dest="format", default="png",
		help="Output image format [default: %default]", metavar="FORMAT")
	parser.add_option(
		"--scale", dest="scale", nargs=2, type="int",
	...
	parser.add_option(
		"-c", "--cookie", dest="cookies", action="append",
		help="""Add this cookie. Use multiple times for more cookies. 
		Specification is value of a Set-Cookie HTTP response header.""",
		metavar="COOKIE")
	parser.add_option(
		"-w", "--wait", dest="wait", default=0, type="int",
		help="Time to wait after loading before the screenshot is taken [default: %default]", metavar="SECONDS")
	parser.add_option(
		"-t", "--timeout", dest="timeout", default=0, type="int",
	...
	parser.add_option("--debug", action="store_true", dest="debug", help="Show debugging information.", default=False)
	parser.add_option("--log", action="store", dest="logfile", default=LOG_FILENAME, help="Select the log output file")
	...
	if options.xvfb:
		...
		try:
			...
			print("Error - Unable to find '%s' for -x/--xvfb option".format(newArgs[0]))
			sys.exit(1)
